...
async def add_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text(strings.ADMIN_ADD_MATRIC, reply_markup=keyboards.get_cancel_menu())
...
async def add_matric(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(strings.ADMIN_ADD_NAME, reply_markup=keyboards.get_cancel_menu())
...
async def add_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(strings.ADMIN_ADD_IC, reply_markup=keyboards.get_cancel_menu())
...
async def add_ic(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(strings.ADMIN_ADD_PROG, reply_markup=keyboards.get_cancel_menu())
...
async def add_prog(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    loading = await update.message.reply_text(strings.ADMIN_SAVING)
    ...
    try:
        if db.add_member(name, matric, ic, prog):
            await loading.edit_text(strings.ADMIN_ADD_SUCCESS.format(name=name, matric=matric), parse_mode="Markdown")
             ...
             await loading.edit_text(strings.ERR_DB_CONNECTION)
    except Exception as e:
        logger.error(e)
        await loading.edit_text(strings.ERR_DB_CONNECTION)
...
async def del_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text(strings.ADMIN_DEL_START, reply_markup=keyboards.get_cancel_menu())
...
async def del_matric(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    loading = await update.message.reply_text(strings.ADMIN_SEARCHING)
    ...
    try:
        ...
        if success:
            ...
            await loading.edit_text(strings.ADMIN_DEL_NOT_FOUND)
    except Exception as e:
        logger.error(e)
        await loading.edit_text(strings.ERR_DB_CONNECTION)
